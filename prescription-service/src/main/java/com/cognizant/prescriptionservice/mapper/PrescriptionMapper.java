...
//				.stream()
//				.map(m ->
//					new PrescriptionMedicineRequest(
//						m.getMedicineId(),
//						m.getMedicineName(),
//						m.getDosage(),
//						m.getFrequency(),
...
//				.getLabTests()
//				.stream()
//				.map(l -> new PrescriptionLabTestRequest(l.getTestName(), l.getNotes()))
//				.toList()
//		);
//
//		return response;
        ...
        if (request.getMedicines() != null) {
            Set<PrescriptionMedicine> medicines = request
                    ...
                    .map(m -> {
                        ...
                        pm.setDuration(m.getDuration());
                        pm.setInstructions(m.getInstructions());
                        pm.setQuantity(m.getQuantity() != null ? m.getQuantity() : 1); // add quantity
                        pm.setPrescription(prescription);
                        return pm;
                    })
                    .collect(Collectors.toSet());
