...
public class LabController {
    ...
    public ResponseEntity<ApiResponse<List<LabTestResponse>>> createTest(
        ...
        if(!labTest.isEmpty() && labTest != null){
            return ResponseEntity.ok(
                    new ApiResponse<>(200, "Lab tests created successfully", labTest)
            );
        } else {
            return ResponseEntity.status(500).body(
                    new ApiResponse<>(500, "Failed to create lab test", null)
            );
        }
    }
    ...
    public ResponseEntity<ApiResponse<List<LabTestResponse>>> getPendingTests(
        ...
        if (!roles.contains("LAB_TECHNICIAN") && !roles.contains("ADMIN")) {
            throw new InvalidRoleException("Invalid Role: Only users with LAB_TECHNICIAN role can view pending lab tests");
        }
        List<LabTestResponse> labTests = labTestService.getPendingLabTests();
        if(!labTests.isEmpty() && labTests != null){
            return ResponseEntity.ok(
                    new ApiResponse<>(200, "Pending lab tests fetched successfully", labTests)
            );
        }
        else{
            return ResponseEntity.status(404).body(
                    new ApiResponse<>(404, "No pending lab tests found", null)
            );
        }
    }
    ...
    public ResponseEntity<ApiResponse<LabTestResponse>> collectSample(
        ...
        if(response != null){
            return ResponseEntity.ok(
                    new ApiResponse<>(200, "Sample collected successfully", response)
            );
        }
        else{
            return ResponseEntity.status(404).body(
                    new ApiResponse<>(404, "Lab test not found with id: " + id, null)
            );
        }
    ...
    public ResponseEntity<ApiResponse<LabTestResponse>> startTest(
        ...
        if(response != null){
            return ResponseEntity.ok(
                    new ApiResponse<>(200, "Lab test started successfully", response)
            );
        }
        else{
            return ResponseEntity.status(404).body(
    ...
    public ResponseEntity<ApiResponse<LabResultResponse>> uploadResult(
        ...
        if(response != null){
            return ResponseEntity.ok(
                    new ApiResponse<>(200, "Lab result uploaded successfully", response)
            );
        }
        else{
            return ResponseEntity.status(404).body(
    ...
    public ResponseEntity<ApiResponse<LabResultResponse>> getLabResults(
        ...
        if(responses != null) {
            return ResponseEntity.ok(
                    new ApiResponse<>(200, "Lab result fetched successfully for lab test", responses)
            );
        } else {
            return ResponseEntity.status(404).body(
                    new ApiResponse<>(404, "No lab result found for lab test with id: " + id, null)
            );
        }
    }
    ...
    public ResponseEntity<ApiResponse<List<LabResultResponse>>> getResultsByPatientId(
        ...
        if(!responses.isEmpty() && responses != null) {
            return ResponseEntity.ok(
                    new ApiResponse<>(200, "Lab results fetched successfully for patient", responses)
            );
        } else {
            return ResponseEntity.status(404).body(
                    new ApiResponse<>(404, "No lab results found for patient with id: " + patientId, null)
            );
        }
    }
}
