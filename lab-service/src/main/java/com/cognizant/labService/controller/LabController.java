...
public class LabController {
    ...
    @GetMapping("/all")
    public ResponseEntity<ApiResponse<List<LabTestResponse>>> getAllTests(
            @RequestHeader("X-User-Role") String roles
    ) {
        if (!roles.contains("LAB_TECHNICIAN") && !roles.contains("ADMIN")) {
            throw new InvalidRoleException("Only LAB_TECHNICIAN or ADMIN can view all tests");
        }
        List<LabTestResponse> all = labTestService.getAllLabTests();
        if (all != null && !all.isEmpty()) {
            return ResponseEntity.ok(new ApiResponse<>(200, "All lab tests fetched", all));
        } else {
            return ResponseEntity.status(404).body(new ApiResponse<>(404, "No lab tests found", null));
        }
    }
