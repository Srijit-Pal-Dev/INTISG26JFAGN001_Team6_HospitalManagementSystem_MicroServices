...
public class LabTestServiceImpl implements LabTestService {
    ...
    public List<LabTestResponse> createLabTests(CreateLabTestRequest request) {
        ...
        List<LabTestResponse> responses = savedTests.stream()
                .map(saved -> new LabTestResponse(
                        saved.getId(),
                        saved.getPatientId(),
        ...
        billingClient.updateLabFee(appointmentId, totalFee, responses);
        return responses;
    }
    ...
    public List<LabTestResponse> getPendingLabTests() {
         List<LabTestResponse> result = labTestRepository.findByStatus(LabTestStatus.PENDING)
                .stream()
                .map(mapper::toDto)
                .toList();
         return result;
...
//        }
    ...
    private InvoiceUpdateResponse createInvoice(Long appointmentId, BigDecimal labFee, List<LabTestResponse> labTestResponses) {
        return billingClient.updateLabFee(appointmentId, labFee, labTestResponses);
    }
