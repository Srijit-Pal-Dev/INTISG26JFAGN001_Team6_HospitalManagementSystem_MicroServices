...
class MediclaimServiceImplTest {
	...
	@BeforeEach
	void setUp() {
		invoice = Invoice.builder().id(1L).patientId(1L).totalAmount(new BigDecimal("1000")).build();

		payment = Payment.builder().id(1L).invoice(invoice).transactionId("TXN123").patientId(1L).build();
	...
	void updateMediclaimStatus_Approved() {
		...
		MediclaimDTO result = mediclaimService.updateMediclaimStatus(100L, 1L, MediclaimStatus.APPROVED);
	...
	void updateMediclaimStatus_Rejected() {
		...
		MediclaimDTO result = mediclaimService.updateMediclaimStatus(100L, 1L, MediclaimStatus.REJECTED);
	...
	void updateMediclaimStatus_Pending_NoNotification() {
		...
		MediclaimDTO result = mediclaimService.updateMediclaimStatus(100L, 1L, MediclaimStatus.PENDING);
	...
	void updateMediclaimStatus_NotFound() {
		...
		assertThrows(
			...
			() -> mediclaimService.updateMediclaimStatus(100L, 99L, MediclaimStatus.APPROVED)
		);
	}
	...
	void getAllMediclaimsByPatientId_Empty() {
		...
		assertThrows(ResourceNotFoundException.class, () -> mediclaimService.getAllMediclaimsByPatientId(99L));
	}
